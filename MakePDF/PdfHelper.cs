///A Small Library For Creating Pdf files.
...
namespace PdfHelper
{
	...
	public class InfoDict:PdfObject
	{
		...
		{
			info=string.Format("{0} 0 obj<</ModDate({1})/CreationDate({1})/Title({2})/Creator(ThanTT)"+
                "/Author({3})/Producer(ThanTT)/Company({4})>>\rendobj\r",
